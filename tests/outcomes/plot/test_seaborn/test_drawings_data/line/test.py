...
class TestSeabornLine(PlottingTest):
    ...
    def test(self):
        ...
        if len(self.all_figures) != 3:
            return wrong(f'Expected 3 plots to be plotted using seaborn library, found {len(self.all_figures)}')
        ...
        for i in range(len(self.all_figures)):
            ...
            correct_x = [2, 4, 6] if i > 0 else correct_x
...
class Test(unittest.TestCase):
    def test(self):
        status, feedback = TestSeabornLine('main').run_tests()
