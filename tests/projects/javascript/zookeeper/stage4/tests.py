        ...
        for index in included_animals:
            if animal_index[index][0].strip() not in reply:
                return CheckResult.wrong(
                    "The " + animal_index[index][1] + " wasn't printed but was expected"
                )

        for index in excluded_animals:
            if animal_index[index][0].strip() in reply:
                return CheckResult.wrong(
                    "The " + animal_index[index][1] + " was printed but wasn't expected"
                )

        if the_end_message not in reply:
            return CheckResult.wrong(
                "You should print '{}' at the end of the program".format(
                    the_end_message
                )
            )
