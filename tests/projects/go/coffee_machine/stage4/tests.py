    ...
    def check(self, reply: str, clue: Any) -> CheckResult:
        ...
        if len(milk) != 2:
            return CheckResult.wrong(
                'There should be two lines with "milk", ' + f"found: {len(milk)}"
            )

        if len(water) != 2:
            return CheckResult.wrong(
                'There should be two lines with "water", ' + f"found: {len(water)}"
            )

        if len(beans) != 2:
            return CheckResult.wrong(
                'There should be two lines with "beans", ' + f"found: {len(beans)}"
            )

        if len(cups) != 2:
            return CheckResult.wrong(
                'There should be two lines with "cups", ' + f"found: {len(cups)}"
            )
        ...
        if action == "take":
            if diff(milk) != 0:
                return CheckResult.wrong(
                    'After "take" action milk ' + "amount shouldn't be changed"
                )

            if diff(water) != 0:
                return CheckResult.wrong(
                    'After "take" action water ' + "amount shouldn't be changed"
                )

            if diff(beans) != 0:
                return CheckResult.wrong(
                    'After "take" action beans ' + "amount shouldn't be changed"
                )

            if diff(cups) != 0:
                return CheckResult.wrong(
                    'After "take" action cups ' + "amount shouldn't be changed"
                )

            if money[1] != 0:
                return CheckResult.wrong(
                    'After "take" action money ' + "amount should be zero"
                )
            ...
            if diff(money) != 0:
                return CheckResult.wrong(
                    'After "fill" action ' + "money amount should not be changed"
                )
